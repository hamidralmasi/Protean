...
for USE_MULTI_PRIO in 1; do
	for BURST_SIZES in 0.5 0.75 1;do
						...
						for BURST_FREQ in 1 2 3;do
							for BUILDUP_THRESH in 2.25;do
								for ALG in $DT $CS $PROTEAN;do
									BURST_SIZE=$(python3 -c "print($BURST_SIZES*$BUFFER)")
									FLOWFILE="$DUMP_DIR/fcts-single-large-$TCP-$ALG-$LOAD-$BURST_SIZES-$BURST_FREQ-$PROTEAN_BETA-$USE_NORMALIZED_DQDT-$USE_MAX_DQDT-$USE_MULTI_PRIO-$BUILDUP_THRESH.fct"
									TORFILE="$DUMP_DIR/tor-single-large-$TCP-$ALG-$LOAD-$BURST_SIZES-$BURST_FREQ-$PROTEAN_BETA-$USE_NORMALIZED_DQDT-$USE_MAX_DQDT-$USE_MULTI_PRIO-$BUILDUP_THRESH.stat"
									ALGFILE="$DUMP_DIR/alg-single-large-$TCP-$ALG-$LOAD-$BURST_SIZES-$BURST_FREQ-$PROTEAN_BETA-$USE_NORMALIZED_DQDT-$USE_MAX_DQDT-$USE_MULTI_PRIO-$BUILDUP_THRESH.stat"
