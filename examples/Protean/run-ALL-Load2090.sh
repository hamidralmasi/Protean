	...
	for USE_HIGH_PRIO_SHORTS in 1;do
		for USE_MULTI_PRIO in 1; do
			for BURST_SIZES in 0.125 0.25 0.375 0.5 0.75;do
				for LOAD in 0.2 0.9;do
					# for USE_MAX_DQDT in 0 1; do
					 	# for USE_NORMALIZED_DQDT in 0 1;do
							for PROTEAN_BETA in 0.25;do
								for BURST_FREQ in 1 2;do
									for BUILDUP_THRESH in 2.25;do
										for ALG in ${BUF_ALGS[@]};do
											BURST_SIZE=$(python3 -c "print($BURST_SIZES*$BUFFER)")
											FLOWFILE="$DUMP_DIR/fcts-single-large-$TCP-$ALG-$LOAD-$BURST_SIZES-$BURST_FREQ-$PROTEAN_BETA-$USE_NORMALIZED_DQDT-$USE_MAX_DQDT-$USE_MULTI_PRIO-$USE_HIGH_PRIO_SHORTS-$BUILDUP_THRESH.fct"
											TORFILE="$DUMP_DIR/tor-single-large-$TCP-$ALG-$LOAD-$BURST_SIZES-$BURST_FREQ-$PROTEAN_BETA-$USE_NORMALIZED_DQDT-$USE_MAX_DQDT-$USE_MULTI_PRIO-$USE_HIGH_PRIO_SHORTS-$BUILDUP_THRESH.stat"
											ALGFILE="$DUMP_DIR/alg-single-large-$TCP-$ALG-$LOAD-$BURST_SIZES-$BURST_FREQ-$PROTEAN_BETA-$USE_NORMALIZED_DQDT-$USE_MAX_DQDT-$USE_MULTI_PRIO-$USE_HIGH_PRIO_SHORTS-$BUILDUP_THRESH.stat"
											while [[ $(ps aux | grep evaluation-optimized | wc -l) -gt 60 ]];do
												sleep 30;
