...
InvokeAlgStats(Ptr<OutputStreamWrapper> stream, uint32_t BufferSize, uint32_t leafId, double nanodelay) {
	...
	for (uint32_t i = 0; i < queues.GetN(); i++) {
		Ptr<GenQueueDisc> genDisc = DynamicCast<GenQueueDisc>(queues.Get(i));
		if (genDisc->getPortId() == 21) {
			//Port 1 stats here
			p1 = genDisc;

		}
		else if (genDisc->getPortId() == 20) {
			//Port 2 stats here
			p2 = genDisc;
		}
	}
	...
	*stream->GetStream()
			...
			<< " " << p1->getInstDqDt(fixed_prio)
			<< " " << p1->getEWMADqDt(fixed_prio)
			<< " " << p1->getMaxDqDt(fixed_prio)
			...
			<< " " << p2->getInstDqDt(fixed_prio)
			<< " " << p2->getEWMADqDt(fixed_prio)
			<< " " << p2->getMaxDqDt(fixed_prio)
                           ...
                           long &flowCount, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME, int numPrior)
{
	for (int txServer = 0; txServer < incastDegree1 + incastDegree2 ; txServer++) {
		uint64_t flowSize = 100000; //100MB

		uint32_t prior =  fixed_prio; // rand_range(1, numPrior - 1);
		Time startTime = Seconds(START_TIME) + NanoSeconds (150) + MicroSeconds(rand_range(50, 100));
