...
results_path = "/home/evl/halmas3/data/buffer_sizing_v2/simulator/ns-3.35/examples/Protean/Sensitivity-Load-3sec/results"
sns.set(style='ticks')
                                                                                ...
                                                                                for use_high_prio_shorts in ["1"]:
                                                                                        for buildup_thresh in ["0.625", "1.25", "2.5", "3.75", "5"]:
                                                                                                ...
                                                                                                if (buildup_thresh == "0.625"):
                                                                                                        ...
                                                                                                        c_bt = "4"
                                                                                                data.append([c_load, fct999, fct99, fct95, fctavg, buffer, burst_size, protean_beta, c_bt, burst_freq])

df = pd.DataFrame(data, columns=["load", "fct999", "fct99", "fct95", "fctavg", "buffer", "burst_size", "protean_beta", "buildup_thresh", "burst_freq"])

# TCP ######################################################################################################################

df_beta = df[(df["burst_size"] == "0.75") & (df["buffer"] == "9.6")& (df["burst_freq"] == "1")&(df["buildup_thresh"] == "2")]
sns.lineplot(data = df_beta ,x = "load" ,y = "fct999", hue="protean_beta", style = "protean_beta", markers = ["8", "s", "D", "^"], palette = palette, dashes = ['','','',''])
...
plt.tight_layout()
plt.savefig(results_path + "/sensitivity-fct999-beta" + ".png")
plt.figure()
